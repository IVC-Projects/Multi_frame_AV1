...
from VDSRUNK_Single_ra import BATCH_SIZE
from VDSRUNK_Single_ra import PATCH_SIZE
...
def prepare_nn_data(train_list):
    ...
    for i in batchSizeRandomList:
        ...
        # l = highData_image.astype('uint8')
        # l = Image.fromarray(l)
        # l.show()
        gt_image = c_getYdata(train_list[i][1])
        # l = gt_image.astype('uint8')
        # l = Image.fromarray(l)
        # l.show()
        ...
        #crop images to the disired size.
            ...
            # l = highData_imgY.astype('uint8')
            # l = Image.fromarray(l)
            # l.show()
            # l = gt_imgY.astype('uint8')
            # l = Image.fromarray(l)
            # l.show()
        #normalize
...
def getWH(yuvfileName):
    ...
    # print(deyuvFilename)
    # print(os.path.basename(deyuvFilename).split('_')[-2])
    # print(os.path.basename(deyuvFilename).split('_')[1])
    # print(os.path.basename(deyuvFilename).split('_')[-3])
    # print(os.path.basename(deyuvFilename).split('_')[-5])
    # print(os.path.basename(deyuvFilename).split('_')[-1])
    if 'x' in os.path.basename(deyuvFilename).split('_')[-2]:
        ...
        wxh = os.path.basename(deyuvFilename).split('_')[1]
    elif 'x' in os.path.basename(deyuvFilename).split('_')[-3]:
        wxh = os.path.basename(deyuvFilename).split('_')[-3]
    elif 'x' in os.path.basename(deyuvFilename).split('_')[-5]:
        wxh = os.path.basename(deyuvFilename).split('_')[-5]
    else:
        print(yuvfileName)
